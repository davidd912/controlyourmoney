  ...
  return (
    <Card className="border-0 shadow-lg dark:bg-gray-800 dark:border-gray-700">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-semibold text-gray-800 dark:text-white">{t('category_breakdown_title')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {categoryData.length === 0 ? (
          <p className="text-gray-400 dark:text-gray-500 text-center py-4">{t('no_data')}</p>
        ) : (
              ...
              <div key={category} className="space-y-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex justify-between items-start text-sm">
                  <span className="font-medium text-gray-700 dark:text-gray-200">
                    {t(`exp_cat.${category}`, categoryLabels[category] || category)}
                  </span>
                  <div className="text-left" dir="ltr">
                    ...
                    {budget > 0 && (
                      <div className={`text-xs ${isOverBudget ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                        {isOverBudget ? `${t('over_budget_label')}: ` : `${t('remaining_label')}: `}
                ...
                {budget > 0 ? (
                  <>
                    ...
                    <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400">
                      <span>{t('budget_percent', { percent: percentage.toFixed(0) })}</span>
                      {isOverBudget && percentage > 100 && (
                        <span className="text-red-600 dark:text-red-400 font-semibold">
                          {t('over_by_percent', { percent: (percentage - 100).toFixed(0) })}
                        </span>
                      )}
                    </div>
                  </>
                ) : (
                  <p className="text-xs text-gray-400 dark:text-gray-500">{t('no_budget_set')}</p>
                )}
              </div>
            );
          })
