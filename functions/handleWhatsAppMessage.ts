...
- מילים כמו "עזרה", "מה אתה יכול", "תסביר" = help`,
    ...
    if (intent === "add_income") {
      ...
      const searchResult = await base44.asServiceRole.integrations.Core.InvokeLLM({
        prompt: `חפש מידע על: ${aiResponse.search_query}\n\nספק תשובה מקיפה ומדויקת בעברית.`,
        add_context_from_internet: true
      });
      reply = `🔍 *תוצאות חיפוש:*\n\n${searchResult}`;

    } else if (intent === "help") {
      reply = `👋 *שלום! אני Flowli AI העוזר החכם שלך*\n\nאני כאן לעזור לך לנהל את הכספים שלך! 💰\n\n🔍 *מה אני יכול לעשות?*\n\n💰 *ניהול תקציב:*\n• *להוסיף הוצאות והכנסות* (לדוגמה: "הוסף הוצאה 50 שקל בסופר היום")\n• *להציג את נתוני התקציב שלך* (כמה הוצאה, כמה נכנס וכו')\n📊 *לעקוב אחר ההתקדמות החודשית שלך*\n✏️ *לעדכן או למחוק פריטים מהתקציב*\n🌐 *לחפש מידע באינטרנט* (לדוגמה: "חפש בגוגל מניות מומלצות")\n🌍 *לחפש חדשות אקטואליות* (לדוגמה: "מה חדש בעולם הכלכלה?")\n\n💬 פשוט שלח לי הודעה והתחל!`;

    } else {
      reply = `🤔 לא הבנתי את הבקשה. נסה שוב או שלח "עזרה" להסבר מלא על מה אני יכול לעשות.`;
    }
